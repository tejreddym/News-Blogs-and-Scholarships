...
# 1. Change the `<div className="lg:col-span-12">` inside `<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">`
# to `<div className="lg:col-span-8">`
text = re.sub(
    r'(<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">\s*)<div className="lg:col-span-12">',
    r'\1<div className="lg:col-span-8">',
    text,
    count=1
)

# 2. Extract out the entire Top Carousels and the Newsletter component
# We'll just delete them first using regex.
trending_pattern = re.compile(r'(\n|\s)*\{\/\*\s*Trending News Carousel\s*\*\/\}.*?<div className="grid grid-cols-', re.DOTALL)
featured_pattern = re.compile(r'(\n|\s)*\{\/\*\s*Featured Blogs Carousel\s*\*\/\}.*?<div className="grid grid-cols-', re.DOTALL)
newsletter_pattern = re.compile(r'(\n|\s)*\{\/\*\s*Newsletter\s*\*\/\}.*?<\/div>\s*<\/div>\s*<\/main>', re.DOTALL)

text = trending_pattern.sub('\n<div className="grid grid-cols-', text)
text = featured_pattern.sub('\n<div className="grid grid-cols-', text)
text = newsletter_pattern.sub('\n</main>', text)

# Also there was a lg:col-span-12 tag on newsletter, let's catch it if it exists
newsletter_pattern_2 = re.compile(r'(\n|\s)*<div className="lg:col-span-12 bg-secondary-foreground text-white p-8 md:p-12 rounded-3xl relative overflow-hidden mt-10 shadow-2xl">.*?<\/div>\s*<\/div>\s*<\/div>\s*<\/main>', re.DOTALL)
text = newsletter_pattern_2.sub('\n</div></div></main>', text) # Keep the closing divs of the grid

# 3. Reduce grid width xl:grid-cols-4 to xl:grid-cols-3
text = text.replace('xl:grid-cols-4', 'xl:grid-cols-3')

# 4. We need to inject the New Top Carousels right AFTER `<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">`
NEW_CAROUSEL_BLOCK = """
                    {/* NEW TOP CAROUSELS */}
...
# 5. Inject the sidebar right before the final `</div>\n            </main>` which closes the grid and main wrapper.
# Since we stripped Newsletter, the end of file will look like: 
# `</div></div></main>` or similar if we didn't strip carefully.
# Let's cleanly inject it before the last </div> that is wrapped by </main>.
text = re.sub(
    r'(<\/div>\s*)(<\/div>\s*<\/main>\s*<\/div>\s*\)\s*;\s*\}\s*;\s*export default BlogsNews;)',
    r'\1' + SIDEBAR_BLOCK + r'\n\2',
    text,
    count=1
)
